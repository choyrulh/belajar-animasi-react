...
const ChipsetSection = () => {
  ...
  const chipOpacity = useTransform(smoothProgress, [0, 0.1, 0.9, 1], [0, 1, 1, 1]);
  ...
  return (
    <div ref={containerRef} className="relative h-[300vh] bg-black">
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col items-center justify-center perspective-[1200px]">
        ...
        >
          <h2 className="text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-100 via-yellow-400 to-yellow-700 mb-2">
            XERON Z1 CHIP
          </h2>
          <p className="text-blue-400/60 text-xs font-mono tracking-[0.6em] uppercase">
        ...
        <motion.div
          ...
          className="relative w-[300px] h-[300px] md:w-[500px] md:h-[500px] z-20"
        >
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30">
          <div className="relative w-full max-w-6xl h-full flex items-center justify-between px-10">
            
            {/* Left Stats */}
            <motion.div style={{ opacity: statsOpacity, x: useTransform(smoothProgress, [0.4, 0.6], [-50, 0]) }} className="space-y-12">
              <StatItem icon={<Brain size={20}/>} title="Neural Engine" value="32-Core" color="border-yellow-500/50" />
              <StatItem icon={<Layers size={20}/>} title="Transistors" value="25 Billion" color="border-blue-500/50" />
            </motion.div>

            {/* Right Stats */}
            <motion.div style={{ opacity: statsOpacity, x: useTransform(smoothProgress, [0.4, 0.6], [50, 0]) }} className="space-y-12 text-right">
              <StatItem icon={<Gauge size={20}/>} title="Clock Speed" value="4.2 GHz" color="border-purple-500/50" reverse />
              <StatItem icon={<Zap size={20}/>} title="Efficiency" value="+45%" color="border-green-500/50" reverse />
            </motion.div>
...
const StatItem = ({ icon, title, value, color, reverse = false }) => (
  <div className={`flex items-center gap-4 ${reverse ? 'flex-row-reverse' : ''}`}>
    <div className={`w-12 h-12 rounded-xl bg-black/50 backdrop-blur-md border ${color} flex items-center justify-center text-white`}>
      {icon}
    </div>
    <div>
